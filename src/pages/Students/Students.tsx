...
export default function Students() {
  ...
  const deleteStudentMutation = useMutation({
    ...
    onSuccess: (_, id) => {
      toast.success(`xóa thành công students ${id}`)
  ...
  console.log('isLoading', getStudentsQuery.isLoading)
  console.log('isFetching', getStudentsQuery.isFetching)

  // console.log(data?.headers['x-total-count'])
  const totalStudentscCount = Number(getStudentsQuery.data?.headers['x-total-count'] || 0)
  const totalPage = Math.ceil(totalStudentscCount / 10)
                      ...
                      <li key={pageNumber}>
                        <Link
                          className={classNames(
                            'border border-gray-300   py-2 px-3 leading-tight      hover:text-gray-700',
                            {
